...
void RenderFrame(KW_Widget * widget) {
  ...
  if (renderfunc != NULL)
    frame->framerender = renderfunc(KW_GetWidgetRenderer(widget), widget,
                                    tileset, targetgeom.w, targetgeom.h);
  else
    frame->framerender = KW_CreateTileFrameTexture(
        KW_GetWidgetRenderer(widget), tileset, 0, 0, targetgeom.w, targetgeom.h,
        KW_FALSE, KW_FALSE);
}
...
void PaintFrame(KW_Widget * widget, const KW_Rect * absolute, void * data) {
  ...
  if (!KW_QueryWidgetHint(widget, KW_WIDGETHINT_ALLOWTILESTRETCH)) {
    ...
    KW_RenderTileFrame(KW_GetWidgetRenderer(widget),
                       KW_GetWidgetTilesetTexture(widget), 0, 0, &targetgeom,
                       KW_TRUE, KW_TRUE);
  }
}

void FrameGeometryChanged(KW_Widget * widget, const KW_Rect * newgeom,
