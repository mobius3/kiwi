...
void AddWidgetHandler(KW_Widget * widget, KW_WidgetEventHandlerType handlertype,
  ...
  (*count)++;
  widget->eventhandlers[handlertype].handlers = realloc(
      widget->eventhandlers[handlertype].handlers, sizeof(void *) * (*count));
...
void RemoveWidgetHandler(KW_Widget *               widget,
  ...
  for (i = 0; i < *count; i++) {
    ...
    if (j >= 0 && i + 1 < *count) {
      widget->eventhandlers[handlertype].handlers[i] =
          widget->eventhandlers[handlertype].handlers[i + 1];
    }
  }
  ...
  if (*count <= 0) {
    ...
    widget->eventhandlers[handlertype].handlers = realloc(
        widget->eventhandlers[handlertype].handlers, *count * sizeof(void *));
}
