...
KW_Widget * KW_CreateToggle(KW_GUI * gui, KW_Widget * parent,
                            const KW_Rect * geometry) {
  KW_Toggle * tg = AllocToggle();
  KW_Widget * widget =
      KW_CreateWidget(gui, parent, geometry, PaintToggle, DestroyToggle, tg);
  tg->pushed = KW_FALSE;
...
void PaintToggle(KW_Widget * widget, const KW_Rect * absolute, void * data) {
  ...
  if (KW_IsCursorOverWidget(widget))
    column += 6;
  KW_RenderTileFrame(renderer, KW_GetWidgetTilesetTexture(widget), 12, column,
                     &targetgeom, KW_FALSE, KW_FALSE);
}
void DestroyToggle(KW_Widget * widget) {
  KW_Toggle * tg = KW_GetWidgetData(widget, PaintToggle);
  free(tg);
