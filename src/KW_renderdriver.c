...
void KW_BlitSurface(KW_RenderDriver * driver, KW_Surface * src,
                    const KW_Rect * srcRect, KW_Surface * dst,
                    const KW_Rect * dstRect) {
  driver->blitSurface(driver, src, srcRect, dst, dstRect);
}

KW_Surface * KW_CreateSurface(KW_RenderDriver * driver, unsigned width,
                              unsigned height) {
  return driver->createSurface(driver, width, height);
}

void KW_GetSurfaceExtents(KW_RenderDriver * driver, const KW_Surface * surface,
                          unsigned * width, unsigned * height) {
  driver->getSurfaceExtents(driver, surface, width, height);
}

void KW_GetTextureExtents(KW_RenderDriver * driver, KW_Texture * texture,
                          unsigned * width, unsigned * height) {
  driver->getTextureExtents(driver, texture, width, height);
}

void KW_RenderCopy(KW_RenderDriver * driver, KW_Texture * src,
                   const KW_Rect * clip, const KW_Rect * dstRect) {
  driver->renderCopy(driver, src, clip, dstRect);
}

KW_Texture * KW_RenderText(KW_RenderDriver * driver, KW_Font * font,
                           const char * text, KW_Color color,
                           KW_RenderDriver_TextStyle style) {
  return driver->renderText(driver, font, text, color, style);
}

KW_Texture * KW_RenderTextWrapped(KW_RenderDriver * driver, KW_Font * font,
                                  const char * text, KW_Color color,
                                  KW_RenderDriver_TextStyle style,
                                  int                       wrapwidth) {
...
KW_Font * KW_LoadFont(KW_RenderDriver * driver, const char * fontFile,
                      unsigned ptSize) {
  return driver->loadFont(driver, fontFile, ptSize);
}

KW_Font * KW_LoadFontFromMemory(KW_RenderDriver * driver,
                                const void * fontMemory, unsigned long memSize,
                                unsigned ptSize) {
...
void KW_UTF8TextSize(KW_RenderDriver * driver, KW_Font * font,
                     const char * text, unsigned * width, unsigned * height) {
  driver->utf8TextSize(driver, font, text, width, height);
}

unsigned KW_GetPixel(KW_RenderDriver * driver, KW_Surface * surface,
                     unsigned px, unsigned py) {
  return driver->getPixel(driver, surface, px, py);
}

static int imax(int i, int j) { return (i < j) ? j : i; }
