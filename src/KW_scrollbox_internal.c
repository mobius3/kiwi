...
void RenderScrollboxFrame(KW_Scrollbox * sb) {
  ...
  if (sb->framerender != NULL)
    KW_ReleaseTexture(KW_GetWidgetRenderer(widget), sb->framerender);
  sb->framerender =
      KW_CreateTileFrameTexture(KW_GetWidgetRenderer(widget), tileset, 9, 0,
                                targetgeom.w, targetgeom.h, KW_FALSE, KW_FALSE);
}

void PaintScrollboxFrame(KW_Widget * widget, const KW_Rect * absolute,
                         void * data) {
  ...
  if (sb->innercomposite.x != innergeom.x ||
    ...
    scrollgeom.y =
        (unsigned) ((-innergeom.y / innerh) * outergeom.h + TILESIZE);
    ...
    scrollgeom.x =
        (unsigned) ((-innergeom.x / innerw) * outergeom.w + TILESIZE);
  ...
  if (KW_QueryWidgetHint(sb->root, KW_WIDGETHINT_ALLOWTILESTRETCH)) {
    KW_RenderTileFrame(KW_GetWidgetRenderer(sb->root),
                       KW_GetWidgetTilesetTexture(sb->root), 9, 0, &targetgeom,
                       KW_TRUE, KW_TRUE);
  } else {
    if (!sb->framerender)
      RenderScrollboxFrame(sb);
    KW_RenderCopy(KW_GetWidgetRenderer(widget), sb->framerender, NULL,
                  &targetgeom);
  }
}
...
void VerticalBarDrag(KW_Widget * widget, int x, int y, int xrel, int yrel) {
  ...
  KW_ScrollboxVerticalScroll(
      sb->root, (int) (-yrel * (sb->innercomposite.h / outergeom.h * 1.0f)));
}

void ChildrenChange(KW_Widget * widget, KW_WidgetChildrenChangeEvent what,
                    KW_Widget * children) {
...
void RootScrollboxGeometryChange(KW_Widget * widget, const KW_Rect * geometry,
  ...
  sb->innercomposite.x = sb->innercomposite.y = sb->innercomposite.w =
      sb->innercomposite.h = 0;
...
void HorizontalBarDrag(KW_Widget * widget, int x, int y, int xrel, int yrel) {
  ...
  KW_ScrollboxHorizontalScroll(
      sb->root, (int) (-xrel * (sb->innercomposite.w / outergeom.w * 1.0f)));
}
