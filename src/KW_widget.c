...
KW_Widget * KW_CreateWidget(KW_GUI * gui, KW_Widget * parent,
...
KW_Widget * const * KW_GetWidgetChildren(const KW_Widget * widget,
                                         unsigned int *    count) {
...
KW_Texture * KW_GetWidgetTilesetTexture(KW_Widget * widget) {
  return widget->tilesettexture == NULL ? widget->gui->tilesettexture
                                        : widget->tilesettexture;
}

KW_Surface * KW_GetWidgetTilesetSurface(KW_Widget * widget) {
  return widget->tilesetsurface == NULL ? widget->gui->tilesetsurface
                                        : widget->tilesetsurface;
}

void * KW_GetWidgetData(const KW_Widget *      widget,
                        KW_WidgetPaintFunction paint) {
  if (widget->paint != paint)
    return NULL;
...
void KW_GetWidgetAbsoluteGeometry(const KW_Widget * widget,
                                  KW_Rect *         geometry) {
...
void CalculateAbsoluteGeometry(KW_Widget * widget) {
  ...
  KW_GetWidgetAbsoluteGeometry(widget->parent,
                               &parentabs); /* get parent absolute */
...
/* recursively calculate (childs->parent->parent->...->root) all composed
 * geometries.
 *
 * They are relative to its parent, thus, if a child has a .x coordinate less
 * than 0, it is outside the current widget box and it must be recalculated. The
 * current widget parent must also be notified of this change if we happend to
 * be outside parent box */
void CalculateComposedGeometry(KW_Widget * widget) {
  ...
  for (i = 0; i < widget->childrencount; i++) {
    ...
    if (composed.x + composed.w > edges.w)
      edges.w = composed.x + composed.w;
    if (composed.y + composed.h > edges.h)
      edges.h = composed.y + composed.h;
    if (composed.x < 0)
      edges.x += composed.x;
    if (composed.y < 0)
      edges.y += composed.y;
  }
  if (edges.x < widget->geometry.x)
    edges.w += widget->geometry.x - edges.x;
  if (edges.y < widget->geometry.y)
    edges.h += widget->geometry.y - edges.y;
  ...
  if (((widget->composed.x < composed.x) || (widget->composed.y < composed.y) ||
...
void KW_GetWidgetComposedGeometry(const KW_Widget * widget,
                                  KW_Rect *         composed) {
...
void Reparent(KW_Widget * widget, KW_Widget * newparent) {
  ...
  if (newparent != NULL) {
    ...
    newparent->children = realloc(
        newparent->children, newparent->childrencount * sizeof(KW_Widget *));
...
void KW_SetWidgetCustomRenderFunction(KW_Widget *             widget,
...
void KW_PaintWidget(KW_Widget * root) {
  ...
  if (KW_IsDebugWidgetEnabled(root))
    DrawDebugGizmos(root, NULL);
}
...
static void DrawDebugGizmos(KW_Widget * widget, KW_Color * color) {
  ...
  sprintf(buf, "rel: %dx%d+%dx%d, com: %dx%d+%dx%d", widget->geometry.x,
          widget->geometry.y, widget->geometry.w, widget->geometry.h,
          widget->composed.x, widget->composed.y, widget->composed.w,
          widget->composed.h);
  ...
  if (KW_IsCursorOverWidget(widget)) {
    ...
    if (old != widget) {
      if (text)
        KW_ReleaseTexture(renderer, text);
      text = KW_RenderText(renderer, KW_GetFont(KW_GetGUI(widget)), buf,
                           KW_MultiplyColor(widget->debug, 0.5f),
                           KW_TTF_STYLE_NORMAL);
      old = widget;
    }
    ...
    if (dbgrect.y + dbgrect.h > viewport.h)
      dbgrect.y = viewport.h - dbgrect.h;
...
void KW_SetWidgetGeometry(KW_Widget * widget, const KW_Rect * geometry) {
  ...
  KW_ZeroRect(&g);
  if (geometry)
    KW_CopyRect(geometry, &g);
  if (g.w <= 0)
    g.w = 0;
  if (g.h <= 0)
    g.h = 0;
  if ((widget->geometry.x != g.x || widget->geometry.y != g.y ||
       widget->geometry.w != g.w || widget->geometry.h != g.h) ||
...
KW_bool KW_IsCursorPressedOnWidget(KW_Widget * widget) {
  KW_GUI * gui = KW_GetGUI(widget);
  return (gui->cursordown && gui->currentmouseover == widget) ? KW_TRUE
                                                              : KW_FALSE;
}
