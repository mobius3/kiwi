...
static void PaintCheckbox(KW_Widget * widget, const KW_Rect * absolute,
                          void * data);
static void DestroyCheckbox(KW_Widget * widget);
static void GeometryChanged(KW_Widget * widget, const KW_Rect * newgeom,
...
KW_Widget * KW_CreateCheckbox(KW_GUI * gui, KW_Widget * parent,
                              KW_Widget * label, const KW_Rect * geometry) {
  KW_Checkbox * cb = calloc(1, sizeof(KW_Checkbox));
  KW_Widget *   root = KW_CreateWidget(gui, parent, geometry, PaintCheckbox,
                                     DestroyCheckbox, cb);
  cb->label = label;
...
void Reposition(KW_Checkbox * cb, const KW_Rect * newgeom) {
  ...
  togglegeom.h = 30;
  KW_RectFillParentHorizontally(newgeom, geoms, weights, 2, 0,
