...
void MouseUpRadio(KW_Widget* widget, int b) {
  KW_RadioButton * radiobutton = (KW_RadioButton *) KW_GetWidgetUserData(widget);
  int i;
  if(b != 1) {
    return;
  }
  ...
  for(i = 0; i< radiobutton->manager->current_items; i++){
      KW_SetLabelIcon(radiobutton->manager->items[i], &radiobutton->manager->unselected);
  }
...
KW_Widget * KW_CreateRadio(KW_GUI * gui, KW_Widget * parent, KW_RadioManager * manager, const char * text, int value, const KW_Rect * geometry){
  KW_Label * label = calloc(sizeof(KW_Label), 1);
  KW_RadioButton * radiobutton = calloc(sizeof(KW_RadioButton), 1);
  KW_Widget * widget = KW_CreateWidget(gui, parent, geometry, PaintLabel, DestroyRadio, label);
