 ...
 * A Render Driver wraps and abstract calls to a specific rendering backend. For
 *instance, if you want to port KiWi to your own renderer, you would implement
 *all of the functions listed below.
 *
 * You are responsible for creating a valid KW_RenderDriver struct, and you
 *should make available a function for the user to do so: this provide means to
 *create complex initialization routines, enabling you to decouple KiWi
 *initialization from your game rendering initialization. For example, the SDL2
 *RenderDriver exposes a single function, KW_CreateSDL2RenderDriver(), that
 *receives the renderer and window instance to work with.
 **/
 ...
 */
typedef KW_Texture * (*KW_CreateTextureFunction)(KW_RenderDriver * driver,
                                                 KW_Surface *      src);
 ...
 */
typedef KW_Texture * (*KW_LoadTextureFunction)(KW_RenderDriver * driver,
                                               const char *      file);
 ...
 * \return  a KW_Surface.
 */
typedef KW_Surface * (*KW_LoadSurfaceFunction)(KW_RenderDriver * driver,
                                               const char *      file);
typedef void (*KW_ReleaseTextureFunction)(KW_RenderDriver * driver,
                                          KW_Texture *      texture);
typedef void (*KW_ReleaseSurfaceFunction)(KW_RenderDriver * driver,
                                          KW_Surface *      surface);
typedef void (*KW_ReleaseFontFunction)(KW_RenderDriver * driver,
                                       KW_Font *         font);
typedef KW_Surface * (*KW_CreateSurfaceFunction)(KW_RenderDriver * driver,
                                                 unsigned          width,
                                                 unsigned          height);
typedef void (*KW_GetSurfaceExtentsFunction)(KW_RenderDriver *  driver,
                                             const KW_Surface * surface,
                                             unsigned *         width,
                                             unsigned *         height);
typedef void (*KW_GetTextureExtentsFunction)(KW_RenderDriver * driver,
                                             KW_Texture *      texture,
                                             unsigned *        width,
                                             unsigned *        height);
typedef void (*KW_BlitSurfaceFunction)(KW_RenderDriver * driver,
                                       KW_Surface *      src,
                                       const KW_Rect *   srcRect,
                                       KW_Surface *      dst,
                                       const KW_Rect *   dstRect);
typedef void (*KW_SetClipRectFunction)(KW_RenderDriver * driver,
                                       const KW_Rect * clip, int force);
typedef KW_bool (*KW_GetClipRectFunction)(KW_RenderDriver * driver,
                                          KW_Rect *         clip);
typedef unsigned int (*KW_GetPixelFunction)(KW_RenderDriver * driver,
                                            KW_Surface * surface, unsigned px,
                                            unsigned py);
typedef void (*KW_RenderRectFunction)(KW_RenderDriver * driver, KW_Rect * rect,
                                      KW_Color color);
typedef void (*KW_GetViewportSizeFunction)(KW_RenderDriver * driver,
                                           KW_Rect *         rect);
...
extern DECLSPEC void KW_RenderRect(KW_RenderDriver * driver, KW_Rect * rect,
                                   KW_Color color);
extern DECLSPEC void KW_BlitSurface(KW_RenderDriver * driver, KW_Surface * src,
                                    const KW_Rect * srcRect, KW_Surface * dst,
                                    const KW_Rect * dstRect);
extern DECLSPEC KW_Surface * KW_CreateSurface(KW_RenderDriver * driver,
                                              unsigned width, unsigned height);
extern DECLSPEC void         KW_GetSurfaceExtents(KW_RenderDriver *  driver,
                                                  const KW_Surface * surface,
                                                  unsigned * width, unsigned * height);
extern DECLSPEC void         KW_GetTextureExtents(KW_RenderDriver * driver,
                                                  KW_Texture *      texture,
                                                  unsigned * width, unsigned * height);
extern DECLSPEC void KW_RenderCopy(KW_RenderDriver * driver, KW_Texture * src,
                                   const KW_Rect * clip,
                                   const KW_Rect * dstRect);
extern DECLSPEC KW_Texture * KW_RenderText(KW_RenderDriver * driver,
                                           KW_Font * font, const char * text,
                                           KW_Color                  color,
                                           KW_RenderDriver_TextStyle style);
extern DECLSPEC              KW_Texture *
                             KW_RenderTextWrapped(KW_RenderDriver * driver, KW_Font * font,
                                                  const char * text, KW_Color color,
                                                  KW_RenderDriver_TextStyle style, int wrapwidth);
extern DECLSPEC KW_Font * KW_LoadFont(KW_RenderDriver * driver,
                                      const char * fontFile, unsigned ptSize);
extern DECLSPEC KW_Font * KW_LoadFontFromMemory(KW_RenderDriver * driver,
                                                const void *      fontMemory,
                                                unsigned long     memSize,
                                                unsigned          ptSize);
extern DECLSPEC KW_Texture * KW_CreateTexture(KW_RenderDriver * driver,
                                              KW_Surface *      surface);
extern DECLSPEC KW_Texture * KW_LoadTexture(KW_RenderDriver * driver,
                                            const char *      file);
extern DECLSPEC KW_Surface * KW_LoadSurface(KW_RenderDriver * driver,
                                            const char *      file);
extern DECLSPEC void         KW_ReleaseTexture(KW_RenderDriver * driver,
                                               KW_Texture *      texture);
extern DECLSPEC void         KW_ReleaseSurface(KW_RenderDriver * driver,
                                               KW_Surface *      surface);
extern DECLSPEC void KW_ReleaseFont(KW_RenderDriver * driver, KW_Font * font);
extern DECLSPEC KW_bool KW_GetClipRect(KW_RenderDriver * driver,
                                       KW_Rect *         clip);
extern DECLSPEC void    KW_GetViewportSize(KW_RenderDriver * driver,
                                           KW_Rect *         rect);
extern DECLSPEC void    KW_SetClipRect(KW_RenderDriver * driver,
                                       const KW_Rect * clip, int force);
extern DECLSPEC void    KW_ReleaseRenderDriver(KW_RenderDriver * driver);
extern DECLSPEC void KW_UTF8TextSize(KW_RenderDriver * driver, KW_Font * font,
                                     const char * text, unsigned * width,
                                     unsigned * height);
extern DECLSPEC unsigned int KW_GetPixel(KW_RenderDriver * driver,
                                         KW_Surface * surface, unsigned x,
                                         unsigned y);
