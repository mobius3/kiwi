...
void PaintEditbox(KW_Widget * widget, const KW_Rect * absolute, void * data) {
  ...
  int basec = 0;
  if (editbox->mouseover || editbox->active)
    basec = 3;
  if (editbox->clicked)
    basec = 0;
  ...
  KW_RenderTileFrame(renderer, tileset, 6, basec, &targetgeom, KW_FALSE,
                     KW_FALSE);
...
void PaintEditboxText(KW_Editbox * editbox, KW_Rect * _dst) {
  ...
  src.x = src.y = 0;
  src.w = editbox->textwidth;
  src.h = editbox->textheight;

  /* will paint label in the middle */
  dst.y = dst.y + dst.h / 2 - src.h / 2;

  /* clip bottom part (middle, top) */
  if (dst.y + src.h > orig.y + orig.h)
    src.h = orig.h + (orig.y - dst.y) - src.y;
...
void RenderEditboxText(KW_Editbox * editbox) {
  if (editbox->textrender != NULL) {
    KW_ReleaseTexture(KW_GetWidgetRenderer(editbox->widget),
                      editbox->textrender);
  }
  /* use our own font */
  editbox->textrender = KW_RenderText(
      KW_GetWidgetRenderer(editbox->widget), KW_GetEditboxFont(editbox->widget),
      editbox->text, KW_GetEditboxTextColor(editbox->widget),
      KW_TTF_STYLE_NORMAL);

  if (editbox->textrender != NULL)
    KW_GetTextureExtents(KW_GetWidgetRenderer(editbox->widget),
                         editbox->textrender, &(editbox->textwidth),
                         &(editbox->textheight));
  else
    KW_UTF8TextSize(KW_GetWidgetRenderer(editbox->widget),
                    KW_GetEditboxFont(editbox->widget), "",
                    &(editbox->textwidth), &(editbox->textheight));
}
...
void AdjustCursor(KW_Editbox * editbox, int cursormove) {
  ...
  if (cursormove > 0) {
    ...
    while (cursormove-- > 0) {
      (void) (isutf(editbox->text[++(editbox->cursor)]) ||
              isutf(editbox->text[++(editbox->cursor)]) ||
              isutf(editbox->text[++(editbox->cursor)]) || ++(editbox->cursor));
    }
  } else if (cursormove < 0) {
    while (cursormove++ < 0) {
      (void) (isutf(editbox->text[--(editbox->cursor)]) ||
  ...
  KW_UTF8TextSize(KW_GetWidgetRenderer(editbox->widget),
                  KW_GetEditboxFont(editbox->widget), editbox->text,
                  (unsigned *) &(editbox->cursorx), NULL);
