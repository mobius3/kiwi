...
typedef void (*KW_WidgetPaintFunction)(KW_Widget *     widget,
                                       const KW_Rect * absolute, void * data);
typedef void (*KW_WidgetDestroyFunction)(KW_Widget * widget);

typedef KW_Texture * (*KW_CustomRenderFunction)(KW_RenderDriver * renderer,
                                                KW_Widget *       widget,
                                                KW_Surface * tileset, int w,
                                                int h);
...
typedef void (*KW_OnKeyDown)(KW_Widget * widget, SDL_Keycode sym,
                             SDL_Scancode code);
typedef void (*KW_OnKeyUp)(KW_Widget * widget, SDL_Keycode sym,
                           SDL_Scancode code);

/* widget internal events */
typedef void (*KW_OnGeometryChange)(KW_Widget * widget, const KW_Rect * newgeom,
...
typedef void (*KW_OnWidgetChildrenChange)(KW_Widget *                  widget,
                                          KW_WidgetChildrenChangeEvent what,
                                          KW_Widget *                  child);
 ...
 * rendering on top of it. It also uniquely identifies the kind of created
 *          widget (meaning that the paint function address is taken as the
 * widget type).
 *
 *          Widget geometry is relative to its parent. Consider the following
 * figure:
 *
 * \verbatim
 *
 ...
 * \endverbatim
 *
 *          The root widget starts at 10x10. The child widget starts at 10x30
 * *relative* to the root widget. That means that its absolute position is
 * actually 20x40! The 'Child's Child' widget starts at 40x20 relative to its
 * parent but its absolute position is (10 + 10 + 40, 10 + 30 + 20) = 60x60. If
 * the parent widget moves, all its children move together.
 *
 *          Widgets also have a composed geometry that is calculated by
 * considering each child geometry. They represent the total area occupied by a
 * widget and its children. In the above picture, the 'Root's Child' widget
 * composed geometry is calculated by taking each of its children composed
 * geometry and checking if they are out of the current widget composed
 * geometry. Since 'Child's Child' width and height is out of 'Root's Child'
 *          geometry, 'Root's Child' composed geometry ends up not being
 * 10x30+70x90, but rather 10x30+(40+80+70x90) = 10x30+120x90. Note the width
 * was updated but not the height.
 *
...
extern DECLSPEC KW_Widget *
                KW_CreateWidget(KW_GUI * gui, KW_Widget * parent, const KW_Rect * geometry,
                                ...
                                KW_WidgetDestroyFunction widgetdestroy, void * data);
 ...
 * and in the new parent widget. The composed geometry also needs to be updated
 * in both parents in a chain that might go as high as the root widget. You
 * should avoid widget reparenting if you can.
 *
 *          To make the widget a top-level widget you should pass \c NULL as the
 * @p parent parameter.
 *
 ...
 */
extern DECLSPEC KW_Widget * const *
                KW_GetWidgetChildren(const KW_Widget * widget, unsigned int * count);
...
KW_SetWidgetCustomRenderFunction(KW_Widget *             widget,
                                 KW_CustomRenderFunction renderfunction);
 ...
 * \details Iterates through all of the @p widget subtree searching for a widget
 to put it at the back of chidlren to render makeing it effectivly be on top.
 ...
 * KW_WIDGETHINT_HIDDEN)* \param   widget The widget that will now be painted.
 */
 ...
 */
extern DECLSPEC void KW_EnableWidgetDebug(KW_Widget * widget,
                                          KW_bool     enableInChildren);
 ...
 */
extern DECLSPEC void KW_DisableWidgetDebug(KW_Widget * widget,
                                           KW_bool     disableInChildren);
...
extern DECLSPEC void KW_EnableWidgetHint(KW_Widget * widget, KW_WidgetHint hint,
                                         KW_bool down);
extern DECLSPEC void KW_DisableWidgetHint(KW_Widget *   widget,
                                          KW_WidgetHint hint, KW_bool down);
extern DECLSPEC KW_bool KW_QueryWidgetHint(const KW_Widget * widget,
                                           KW_WidgetHint     hint);
 ...
 */
extern DECLSPEC void
KW_AddWidgetGeometryChangeHandler(KW_Widget *         widget,
 ...
 */
extern DECLSPEC void
KW_RemoveWidgetGeometryChangeHandler(KW_Widget *         widget,
                                     KW_OnGeometryChange handler);
 ...
 */
extern DECLSPEC void
KW_AddWidgetChildrenChangeHandler(KW_Widget *               widget,
 ...
 */
extern DECLSPEC void
KW_RemoveWidgetChildrenChangeHandler(KW_Widget *               widget,
                                     KW_OnWidgetChildrenChange handler);
...
extern DECLSPEC void
KW_AddWidgetTilesetChangeHandler(KW_Widget *              widget,
                                 KW_OnWidgetTilesetChange handler);
extern DECLSPEC void
KW_RemoveWidgetTilesetChangeHandler(KW_Widget *              widget,
                                    KW_OnWidgetTilesetChange handler);
 ...
 */
extern DECLSPEC void KW_SetWidgetTilesetSurface(KW_Widget *  widget,
                                                KW_Surface * tileset);
 ...
 */
extern DECLSPEC void KW_SetClipChildrenWidgets(KW_Widget * widget,
                                               KW_bool     shouldclip);
