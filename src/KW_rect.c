...
void KW_RectHorizontallyCenterInParent(const KW_Rect * parent,
                                       KW_Rect *       inner) {
  inner->x = (parent->w / 2) - (inner->w / 2);
}

void KW_RectCalculateEnclosingRect(KW_Rect * const rects[], unsigned count,
                                   KW_Rect * outer) {
  unsigned i;
  outer->x = rects[0]->x;
  outer->y = rects[0]->y;
  ...
  for (i = 0; i < count; i++) {
    if (rects[i]->x < outer->x) {
      outer->x = rects[i]->x;
    }
    if (rects[i]->y < outer->y) {
      outer->y = rects[i]->y;
    }
    if (rects[i]->w > outer->w) {
      outer->w = rects[i]->w;
    }
    if (rects[i]->h > outer->h) {
      outer->h = rects[i]->h;
    };
  }
}

void KW_RectLayoutVertically(KW_Rect * rects[], unsigned count, int padding,
...
void KW_RectLayoutHorizontally(KW_Rect * rects[], unsigned count, int padding,
...
void KW_RectFillParentVertically(const KW_Rect * parent, KW_Rect * rects[],
                                 unsigned weights[], unsigned count,
                                 int padding) {
  KW_Rect * inner;
...
void KW_RectFillParentHorizontally(const KW_Rect * parent, KW_Rect * rects[],
                                   unsigned weights[], unsigned count,
                                   int                      padding,
                                   KW_RectVerticalAlignment align) {
  KW_Rect * inner;
...
void KW_CopyRect(const KW_Rect * src, KW_Rect * dst) { *dst = *src; }

void KW_ZeroRect(KW_Rect * rect) { rect->x = rect->y = rect->w = rect->h = 0; }

void KW_MarginRect(const KW_Rect * outer, KW_Rect * inner, int margin) {
  KW_SetRect(inner, margin, margin, outer->w - margin * 2,
             outer->h - margin * 2);
}
