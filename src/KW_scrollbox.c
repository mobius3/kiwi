...
KW_Widget * KW_CreateScrollbox(KW_GUI * gui, KW_Widget * parent,
                               const KW_Rect * geometry) {
  /* creates root scrollbox widget */
  KW_Scrollbox * scrollbox = AllocScrollbox();
  KW_Widget *    outer, *inner;
  KW_Widget * root = KW_CreateWidget(gui, parent, geometry, PaintScrollboxFrame,
  ...
  areageom.x = 0;
  areageom.y = 0; /* areageom.h = 0; areageom.w = 0; */
  ...
  KW_EnableWidgetHint(scrollbox->root, KW_WIDGETHINT_ALLOWTILESTRETCH,
                      KW_FALSE);
...
void KW_ScrollboxVerticalScroll(KW_Widget * scrollbox, int amount) {
  ...
  if (sb->innercomposite.y + amount > 0)
    amount = -sb->innercomposite.y;
...
void KW_ScrollboxHorizontalScroll(KW_Widget * scrollbox, int amount) {
  ...
  KW_GetWidgetGeometry(sb->inner, &geom);
  KW_GetWidgetGeometry(sb->outer, &outer);
  if (sb->innercomposite.x + amount > 0)
    amount = -sb->innercomposite.x;
