...
void ButtonGeometryChanged(KW_Widget * widget, const KW_Rect * newgeom,
                           const KW_Rect * oldgeom);

KW_Widget * KW_CreateButtonAndLabel(KW_GUI * gui, KW_Widget * parent,
                                    const char *    text,
                                    const KW_Rect * geometry) {
  return KW_CreateButton(gui, parent, KW_CreateLabel(gui, NULL, text, geometry),
                         geometry);
}

KW_Widget * KW_CreateButton(KW_GUI * gui, KW_Widget * parent, KW_Widget * label,
                            const KW_Rect * geometry) {
  KW_Rect     labelgeom;
  ...
  button = AllocButton();
  widget = KW_CreateWidget(gui, parent, geometry, PaintButton, DestroyButton,
                           button);
  if (label) {
...
void RenderButton(KW_Widget * widget) {
  ...
  button->normal =
      KW_CreateTileFrameTexture(KW_GetWidgetRenderer(widget), tileset, 3, 3,
                                targetgeom.w, targetgeom.h, KW_FALSE, KW_FALSE);
  if (button->over != NULL)
    KW_ReleaseTexture(driver, button->over);
  button->over = KW_CreateTileFrameTexture(driver, tileset, 3, 0, targetgeom.w,
                                           targetgeom.h, KW_FALSE, KW_FALSE);
}
