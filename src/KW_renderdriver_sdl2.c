...
static KW_Texture * KWSDL_renderText(KW_RenderDriver * driver, KW_Font * font,
                                     const char * text, KW_Color color,
                                     KW_RenderDriver_TextStyle style);
static KW_Texture * KWSDL_renderTextWrapped(KW_RenderDriver * driver,
...
static KW_Font *    KWSDL_loadFont(KW_RenderDriver * driver, const char * font,
                                   unsigned ptSize);
static KW_Font *    KWSDL_loadFontFromMemory(KW_RenderDriver * driver,
...
static void     KWSDL_utf8TextSize(KW_RenderDriver * driver, KW_Font * font,
                                   const char * text, unsigned * width,
                                   unsigned * height);
static unsigned KWSDL_getPixel(KW_RenderDriver * driver, KW_Surface * surface,
                               unsigned px, unsigned py);
static void     KWSDL_renderRect(KW_RenderDriver * driver, KW_Rect * rect,
                                 KW_Color color);
static void     KWSDL_getViewportSize(KW_RenderDriver * driver, KW_Rect * rect);

struct KW_RenderDriver * KW_CreateSDL2RenderDriver(SDL_Renderer * renderer,
                                                   SDL_Window *   window) {
...
static KW_Texture * KWSDL_renderTextWrapped(KW_RenderDriver * driver,
                                            KW_Font * kwfont, const char * text,
                                            KW_Color                  color,
                                            KW_RenderDriver_TextStyle style,
                                            int wrapwidth) {
  KWSDL *       kwsdl = (KWSDL *)driver->priv;
  ...
  sdlcolor.r = color.r, sdlcolor.g = color.g, sdlcolor.b = color.b,
  sdlcolor.a = color.a;
  ...
  if (wrapwidth > 0) {
    textsurface =
        TTF_RenderUTF8_Blended_Wrapped(font, text, sdlcolor, wrapwidth);
  } else {
...
static KW_Texture * KWSDL_renderText(KW_RenderDriver * driver, KW_Font * kwfont,
                                     const char * text, KW_Color color,
                                     KW_RenderDriver_TextStyle style) {
  /* save on duplicated code, this function should probably be removed in the
   * future */
...
static void KWSDL_renderCopy(KW_RenderDriver * driver, KW_Texture * texture,
  ...
  SDL_RenderCopy(kwsdl->renderer, texture->texture, src ? &srcRect : NULL,
                 dst ? &dstRect : NULL);
}
